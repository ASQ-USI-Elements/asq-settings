...
module.exports = ASQPlugin.extend({
  ...
  mixinSettings: coroutine(function* mixinSettingsGen(des, src) {
    var defaultSettings = this.asq.api.defaultSettings.getDefaultSettingsOfLevelAsObject('presentation');
    ...
    for ( var i=0; i<des.length; i++ ) {
      ...
      if ( src.hasOwnProperty(key) ) {
        ...
        try{
          ...
          console.log('Exception(pre): ', e, key, src[key]);
          des[i].value = defaultSettings[key].value;
          des[i].markModified('value');
          yield des[i].save();
        }
      } 
    ...
    for ( var i=0; i<des.length; i++ ) {
      ...
      if ( src.hasOwnProperty(key) ) {
        ...
        try{
          ...
          console.log('Exception(pre)(updating): ', e, key);
          return false
        }
      } 
    }
  ...
  updatePresentationSettingsDB: coroutine(function *updatePresentationSettingsDBGen(slideshow_id, settings){
    ...
    var flag = yield this.mixinSettingsForUpdating(currentSettings, settings);
    return flag
  }),
  ...
  updateSlideshowSettings: coroutine(function *updateSlideshowSettings(option){
    ...
    var oldSettings = yield slideshow.getSettings();

    var flag = yield this.updatePresentationSettingsDB(option.slideshow_id, option.settings);
